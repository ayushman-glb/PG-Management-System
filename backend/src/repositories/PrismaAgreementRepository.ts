...
export class PrismaAgreementRepository implements IAgreementRepository {
  ...
  async createAgreement(data: any): Promise<any> {
    return this.prisma.agreement.create({
      data: {
        agreementNumber: data.agreementNumber || `RMB-AGR-${Date.now().toString().slice(-6)}`,
        ...
        electricityCharges: data.electricityCharges || 'As per Sub-Meter Reading',
        wifiCharges: data.wifiCharges || 'Complimentary High-Speed WiFi',
        foodCharges: data.foodCharges || 'Included in Monthly Rent',
        noticePeriodDays: data.noticePeriodDays || 30,
        refundPolicy: data.refundPolicy || 'Security deposit refundable within 7 days post checkout deduction.',
        houseRules: data.houseRules || ['No loud music after 10 PM', 'No smoking or alcohol'],
        visitorPolicy: data.visitorPolicy || 'Visitors permitted in common lobby till 8:00 PM',
        curfewTime: data.curfewTime || '10:30 PM Gate Locking Time',
        damagePolicy: data.damagePolicy || 'Resident liable for physical property damage repairs.',
        prohibitedActivities: data.prohibitedActivities || ['Smoking', 'Alcohol', 'Illegal Substances'],
        cleanlinessTerms: data.cleanlinessTerms || 'Daily housekeeping provided. Resident must maintain room hygiene.',
        terminationClause: data.terminationClause || 'Agreement terminable by either party with 30-day prior written notice.',
        privacyClause: data.privacyClause || 'Resident data protected under IT Act & DPDP Act 2023.',
        disputeJurisdiction: data.disputeJurisdiction || 'City Civil Courts Jurisdiction',
        witnessName: data.witnessName,
        witnessPhone: data.witnessPhone,
        startDate: new Date(data.startDate || Date.now()),
        endDate: new Date(data.endDate || Date.now() + 365 * 24 * 60 * 60 * 1000),
  ...
  async createVersionSnapshot(agreementId: string, contractHtml: string, changeReason?: string): Promise<any> {
    const existingVersions = await this.prisma.agreementVersion.count({ where: { agreementId } });
