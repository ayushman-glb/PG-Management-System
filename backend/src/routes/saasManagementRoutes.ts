...
router.post('/fines/rules', async (req: Request, res: Response) => {
  try {
    ...
    res.status(201).json({ success: true, message: 'Fine rule created', data: rule });
  } catch (err: any) {
...
router.post('/fines/issue', async (req: Request, res: Response) => {
  try {
    ...
    res.status(201).json({ success: true, message: 'Fine issued to resident', data: fine });
  } catch (err: any) {
    res.status(400).json({ success: false, message: err.message });
  }
});
...
router.put('/fines/:fineId/waive', async (req: Request, res: Response) => {
  try {
    ...
    const fine = await fineEngine.waiveFine(req.params.fineId, ownerId || 'SYSTEM_OWNER');
...
router.get('/fines/resident/:residentId', async (req: Request, res: Response) => {
  try {
...
router.post('/admin/approve-pg/:pgId', async (req: Request, res: Response) => {
  try {
    ...
    res.status(200).json({ success: true, message: 'PG Property approved and published live!', data: pg });
  } catch (err: any) {
    res.status(400).json({ success: false, message: err.message });
  }
});
...
router.post('/account/delete', async (req: Request, res: Response) => {
  try {
    ...
    if (!userId || !otpConfirmation) {
      throw new Error('User ID and OTP confirmation required for account deletion.');
    }
    ...
    res.status(200).json({ success: true, message: 'Account successfully deactivated and soft-deleted in compliance with data retention laws.' });
  } catch (err: any) {
    res.status(400).json({ success: false, message: err.message });
  }
});
