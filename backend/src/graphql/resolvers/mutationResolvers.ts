...
export const mutationResolvers = {
  signAgreement: async (_: any, args: { agreementId: string; input: any }, ctx: GraphQLContext) => {
    const agreement = await ctx.prisma.agreement.findUnique({ where: { id: args.agreementId } });
    if (!agreement) {
    ...
    if (!hmacSecret) {
      throw new Error('FATAL: JWT_SECRET not set — cannot generate agreement signature hash');
    }
    const hashHmac = crypto.createHmac('sha256', hmacSecret)
      .update(`${args.agreementId}:${args.input.signerType}:${args.input.signerName}:${Date.now()}`)
      .digest('hex');
    ...
    await ctx.prisma.signature.create({
      data: {
        ...
        ipAddress: args.input.ipAddress || '127.0.0.1',
        hashHmac
      }
    });
  ...
  changeResidentStatus: async (_: any, args: { residentId: string; status: any; reason?: string }, ctx: GraphQLContext) => {
    return Container.residentManagementService.updateResidentStatus(
      { residentId: args.residentId, status: args.status, reason: args.reason, updatedBy: ctx.user?.userId || 'system' },
      ctx.user?.role || 'OWNER',
      ctx.user?.userId || 'system'
    );
  },
  updateBedStatus: async (_: any, args: { bedId: string; status: any; notes?: string }, ctx: GraphQLContext) => {
  ...
  createBedHold: async (_: any, args: { bedId: string; reason: any; holdStartDate?: string; holdEndDate?: string; notes?: string }, ctx: GraphQLContext) => {
    const res = await Container.residentManagementService.createBedHold(
      {
        bedId: args.bedId,
        reason: args.reason,
        holdStartDate: args.holdStartDate ? new Date(args.holdStartDate) : new Date(),
  ...
  releaseBedHold: async (_: any, args: { holdId: string }, ctx: GraphQLContext) => {
    await Container.residentManagementService.releaseBedHold(args.holdId, ctx.user?.userId || 'system', ctx.user?.role || 'OWNER');
    return true;
  },
  requestRoomTransfer: async (_: any, args: any, ctx: GraphQLContext) => {
    return Container.residentManagementService.createRoomTransferRequest({
      ...
      preferredMoveDate: args.preferredMoveDate ? new Date(args.preferredMoveDate) : undefined
    });
  },
  approveRoomTransfer: async (_: any, args: { requestId: string; targetBedId?: string; scheduledDate?: string; notes?: string }, ctx: GraphQLContext) => {
  ...
  rejectRoomTransfer: async (_: any, args: { requestId: string; rejectionReason: string }, ctx: GraphQLContext) => {
  ...
  completeRoomTransfer: async (_: any, args: { requestId: string }, ctx: GraphQLContext) => {
  ...
  convertRoomType: async (_: any, args: { roomId: string; newType: 'SINGLE' | 'DOUBLE' | 'TRIPLE' }, ctx: GraphQLContext) => {
  ...
  waiveFine: async (_: any, args: { fineId: string; ownerId: string }, ctx: GraphQLContext) => {
  ...
  register: async (_: any, args: any, ctx: GraphQLContext) => {
    ...
    return { success: true, message: 'User registered', accessToken: res.accessToken, refreshToken: res.refreshToken, user: res.user };
  },
  login: async (_: any, args: { identifier: string; password: string }, ctx: GraphQLContext) => {
    const res = await Container.authService.login(args.identifier, args.password);
    return { success: true, message: 'Login successful', accessToken: res.accessToken, refreshToken: res.refreshToken, user: res.user };
  },
  logout: async () => {
    return true;
  },
  ...
  verifyPhoneOTP: async (_: any, args: { phone: string; otp: string }) => {
    const res = await Container.authService.verifyPhoneOtp(args.phone, args.otp);
    return { success: res.success, message: res.message };
  },
