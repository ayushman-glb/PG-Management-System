...
async function bootstrap() {
  try {
    ...
    try {
      ...
      logger.info(`✓ MongoDB Connected | Host: ${mongoHost} | Database: ${dbName} | Latency: ${connectionTimeMs}ms`);
    } catch (e: any) {
      mongoStatus = 'Connection Pending / In-Memory Seed Fallback';
      logger.warn(`⚠️ MongoDB Connection Warning: ${e.message}. Using Repository Mock Fallbacks.`);
    }
    ...
    const shutdown = async (signal: string) => {
      logger.info(`Received ${signal}. Shutting down worker process PID ${process.pid} gracefully...`);
    ...
    logger.error(`❌ Critical Error starting server instance (PID ${process.pid}):`, error.message);
    process.exit(1);
  }
}
