...
export class BillingController {
  ...
  createOrder = catchAsync(async (req: AuthRequest, res: Response) => {
    ...
    const result = await this.billingService.createPaymentOrder(residentId, baseAmount, isInterstate);
    return ApiResponse.success(res, 'Razorpay payment order initialized', result, 201);
  });
  ...
  verifyPayment = catchAsync(async (req: AuthRequest, res: Response) => {
    ...
    return ApiResponse.success(res, 'Payment verified successfully and bed status updated', result);
  });
