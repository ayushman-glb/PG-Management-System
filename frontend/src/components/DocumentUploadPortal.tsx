...
export const DocumentUploadPortal: React.FC = () => {
  ...
  return (
    <div className="w-full space-y-6">
      {/* Role Selector Tabs */}
      <div className="flex justify-between items-center flex-wrap gap-4 p-4 rounded-3xl bg-neutral-900/60 border border-white/10 backdrop-blur-md">
        ...
        <div className="flex items-center gap-2 text-xs text-neutral-400 font-medium">
          <Lock className="w-4 h-4 text-emerald-400" /> AES-256 Cloudinary Encrypted Storage
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {currentDocs.map(doc => (
          <motion.div
            key={doc.id}
            whileHover={{ y: -3 }}
            className="p-5 rounded-2xl bg-neutral-900 border border-white/10 space-y-4 flex flex-col justify-between shadow-xl"
          >
            <div className="space-y-2">
              ...
              <p className="text-xs text-neutral-400">Encrypted Document Token: DOC-{doc.id}</p>
            </div>

            <div className="pt-3 border-t border-white/5 flex items-center justify-between">
              {doc.status === 'VERIFIED' ? (
                <div className="flex gap-2">
                  <button className="px-3 py-1.5 rounded-xl bg-white/10 text-xs font-semibold text-white hover:bg-white/20 flex items-center gap-1">
                    <Eye className="w-3.5 h-3.5" /> View
                  </button>
                  <button className="px-3 py-1.5 rounded-xl bg-rose-500/10 text-xs font-semibold text-rose-400 hover:bg-rose-500/20 flex items-center gap-1">
                ...
                <button
                  ...
                  className="w-full py-2 rounded-xl bg-amber-500 text-neutral-950 text-xs font-bold hover:bg-amber-400 flex items-center justify-center gap-2 shadow-lg shadow-amber-500/10"
                >
