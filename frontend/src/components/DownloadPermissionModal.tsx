        ...
        >
          ...
          <div
            className={`p-5 flex items-center justify-between border-b ${
              darkMode
                ...
                : 'bg-gradient-to-r from-[#f7f7f7] via-[#ffffff] to-[#ffffff] border-[var(--border-main)]'
            }`}
          >
            <div className="flex items-center gap-2.5">
              <div
                className={`p-2 rounded-xl ${
                  darkMode ? 'bg-amber-500/20 text-amber-400' : 'bg-[var(--brand-primary)]/30 text-[var(--brand-primary)]'
                }`}
              >
              ...
              <div>
                ...
                <p className={`text-xs ${darkMode ? 'text-neutral-400' : 'text-[var(--text-muted)]'}`}>
            ...
            <button
              ...
              className={`p-1.5 rounded-full transition-colors cursor-pointer disabled:opacity-40 ${
                darkMode ? 'hover:bg-white/10 text-neutral-400' : 'hover:bg-[var(--border-main)]/50 text-[var(--text-muted)]'
              }`}
            >
              <X className="w-4 h-4" />
            </button>
          ...
          <div className="p-6 space-y-4">
            <div
              className={`p-4 rounded-2xl border space-y-2.5 ${
                darkMode ? 'bg-neutral-950/70 border-white/10' : 'bg-[var(--bg-surface)]/70 border-[var(--border-main)]'
              }`}
            >
              <div className="flex items-start gap-3">
                <FileText className={`w-5 h-5 mt-0.5 flex-shrink-0 ${darkMode ? 'text-amber-400' : 'text-[var(--brand-primary)]'}`} />
                <div className="space-y-1 overflow-hidden">
                  <p className="text-xs font-bold truncate">{documentTitle}</p>
                  <p className={`text-xs font-mono truncate ${darkMode ? 'text-neutral-400' : 'text-[var(--text-muted)]'}`}>
                    {fileName}
                  </p>
                </div>
              </div>

              <div className="pt-2 border-t border-dashed border-neutral-300 dark:border-white/10 space-y-1.5 text-xs">
                <div className="flex justify-between">
                  <span className={darkMode ? 'text-neutral-400' : 'text-[var(--text-muted)]'}>File Format:</span>
                  <span className="font-semibold">{documentType}</span>
                </div>
                <div className="flex justify-between">
                  <span className={darkMode ? 'text-neutral-400' : 'text-[var(--text-muted)]'}>Target Storage:</span>
          ...
          <div
            className={`p-4 border-t flex gap-3 ${
              darkMode ? 'bg-neutral-950/50 border-white/10' : 'bg-[#FDF9F5] border-[var(--border-main)]'
            }`}
          >
            <button
              type="button"
              ...
              className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-bold flex items-center justify-center gap-2 border transition-all cursor-pointer disabled:opacity-50 ${
                darkMode
                  ...
                  : 'bg-white border-[var(--border-main)] text-[var(--text-muted)] hover:bg-neutral-100 hover:text-[var(--text-main)]'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-2.5 px-4 rounded-xl text-xs font-bold flex items-center justify-center gap-2 border transition-all cursor-pointer shadow-md disabled:opacity-50 ${
                darkMode
                  ...
                  : 'bg-[var(--brand-primary)] hover:bg-[#B37850] text-white border-[var(--brand-primary)]'
              }`}
            >
