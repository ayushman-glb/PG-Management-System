...
for (const rf of routeFiles) {
  ...
  // Use regex matching multiline route declarations
  const routeRegex = /router\.(get|post|put|patch|delete)\s*\(\s*['"]([^'"]+)['"]\s*,\s*([\s\S]*?)\);/g;
  let rm;
  let order = 0;
  while ((rm = routeRegex.exec(content)) !== null) {
    ...
    const subPath = rm[2];
    const rest = rm[3].split(',').map(s => s.trim().replace(/\n/g, ' ')).filter(Boolean);
    const handler = rest[rest.length - 1] || 'handler';
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        for (const regex of apiRegexes) {
          ...
          while ((m = regex.exec(line)) !== null) {
            ...
            // Remove ${query} or ${queryString} or ${searchParams} at the end of path
            cleanPath = cleanPath.replace(/\$\{(?:query|queryString|searchParams|filter|params|qs)[^}]*\}/gi, '');
